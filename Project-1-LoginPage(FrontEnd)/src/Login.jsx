...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      setIsSubmitted(true);
      // Handle form submission, such as sending the data to a backend
  ...
  return (
    <div ref={vantaRef} style={{ height: '100vh', width: '100%', color: 'white' }}>
      <form action="#" className='text-center pt-[50%] lg:pt-[15%]' onSubmit={handleSubmit}>
        ...
        >        
        <h1 className='text-[40px] lg:text-[50px] font-bold'>Login</h1>
          ...
          <div className='px-8'>
            ...
            <div className='flex justify-between px-2'>
              <label className='pt-[20px]'><input type="checkbox" className='mx-2'/>Remember me</label>
              <h3 className='font-semibold pt-[20px] text-end'>Forgot Password?</h3>
            </div>

            <div className='mt-[30px]'>
              <button type="submit" className='bg-white w-full text-black rounded-3xl font-semibold py-2 px-3'>Login</button>
            </div>

            {isSubmitted && <p className="text-green-500 mt-4">Login successful!</p>}

            <div className='mt-[20px]'>
              <h3 className='py-2'>Don't have an account?</h3>
              <a href="signup" className='font-bold '>Register</a>
            </div>
          </div>
        </div>
      </form>
